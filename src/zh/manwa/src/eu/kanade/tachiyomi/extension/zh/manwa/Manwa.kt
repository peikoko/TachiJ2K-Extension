...
/**
 * 漫蛙（manwa.me）图源，适配 TachiJ2K lib 1.4。
 *
 * 从 keiyoushi extensions-source 转换：
 * - 移除 @Source 注解和 keiyoushi 专有依赖
 * - kotlinx.serialization 替换为 org.json
 * - getPreferences() 替换为 Injekt
 * - network.client 替换为 super.client
 */
class Manwa :
    HttpSource(),
    ConfigurableSource {
    ...
    override fun latestUpdatesParse(response: Response): MangasPage {
        val responseData = LatestUpdatesDto(JSONObject(response.body.string()))
    ...
    private fun getFilterTags(): LinkedHashMap<String, String> = try {
        val json = JSONObject(preferences.getString(APP_TAG_LIST_KEY, "")!!)
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        ListPreference(screen.context).apply {
            ...
            val list: Array<ImageSourceInfo> = try {
                val jsonArr = JSONArray(preferences.getString(APP_IMAGE_SOURCE_LIST_KEY, "")!!)
                Array(jsonArr.length()) { i ->
                    ImageSourceInfo.fromJson(jsonArr.getJSONObject(i))
                }
            } catch (_: Exception) {

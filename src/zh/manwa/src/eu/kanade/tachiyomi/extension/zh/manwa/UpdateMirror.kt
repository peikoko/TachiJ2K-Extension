    ...
    private fun updateUrl(chain: Interceptor.Chain): Boolean {
        ...
        try {
            chain.proceed(request).use { response ->
                ...
                val extractLks = JSONArray(String(Base64.decode(extractLksBase64, Base64.DEFAULT)))
                val extractLksList = (0 until extractLks.length()).map { extractLks.getString(it) }
                val extractLksJson = JSONArray().apply {
                    for (item in extractLksList) {
                        put(item.trimEnd('/'))
                    }
                }.toString()
